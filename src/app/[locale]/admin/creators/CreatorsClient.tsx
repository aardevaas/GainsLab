...
import { CheckCircle, XCircle, Clock, ChevronDown, ExternalLink, User, ShieldCheck } from 'lucide-react';
import { approveCreator, rejectCreator, setCreatorTier } from './actions';
import { VERIFICATION_TIERS, TIER_LABELS, TIER_COLORS, type VerificationTier } from '@/lib/creators/tiers';
...
type Props = { applications: Application[]; creators: CreatorProfile[] };
...
const STATUS_CONFIG: Record<string, { label: string; color: string; bg: string }> = {
  ...
  verification: { label: 'Verification', color: '#60a5fa', bg: 'rgba(96,165,250,0.1)' },
};
...
const TABS = ['pending', 'approved', 'rejected', 'verification'] as const;

export function CreatorsClient({ applications, creators }: Props) {
  ...
  const counts = Object.fromEntries(
    TABS.map(t => [t, t === 'verification' ? creators.length : applications.filter(a => a.status === t).length])
  );
  ...
  return (
    <div>
        ...
        {TABS.map(t => {
          ...
          return (
            ...
            >
              ...
              {t === 'verification' && <ShieldCheck size={13} />}
              {cfg.label}
              <span style={{
      ...
      {/* List */}
      {tab === 'verification' ? (
        <CreatorTierList creators={creators} isPending={isPending} startTransition={startTransition} />
      ) : filtered.length === 0 ? (
        <div style={{
...
function CreatorTierList({
  ...
  function handleTierChange(creatorId: string, tier: VerificationTier) {
    startTransition(async () => {
      const res = await setCreatorTier(creatorId, tier);
      setFeedback(prev => ({
        ...prev,
        [creatorId]: res.error ? `Error: ${res.error}` : `Now ${TIER_LABELS[tier]}`,
      }));
    });
  }
        ...
        <div key={c.id} style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <p style={{ fontSize: 11, color: 'var(--color-text-muted)', margin: 0, fontFamily: 'var(--font-mono)' }}>
              {c.total_clients} clients{c.avg_client_rating ? ` · ${c.avg_client_rating.toFixed(1)}★` : ''} · /{c.slug}
            </p>
            {feedback[c.id] && (
              <p style={{ fontSize: 11, fontWeight: 600, color: '#4ade80', margin: '4px 0 0' }}>
                {feedback[c.id]}
              </p>
            )}
          </div>
          ...
          <select
            value={c.verification_tier}
            disabled={isPending}
            onChange={e => handleTierChange(c.id, e.target.value as VerificationTier)}
            style={{
              flexShrink: 0, padding: '8px 12px', borderRadius: 10,
              border: `1px solid ${TIER_COLORS[c.verification_tier]}40`,
              background: `${TIER_COLORS[c.verification_tier]}15`,
              color: TIER_COLORS[c.verification_tier],
              fontSize: 12, fontWeight: 700, cursor: isPending ? 'not-allowed' : 'pointer',
            }}
          >
            {VERIFICATION_TIERS.map(t => (
              <option key={t} value={t} style={{ color: '#0a0c0f' }}>{TIER_LABELS[t]}</option>
            ))}
          </select>
        </div>
      ))}
