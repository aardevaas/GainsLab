...
import { syncMyScores } from "@/app/[locale]/(app)/community/actions";
...
export default async function DashboardPage() {
  ...
  const sevenDaysAgoIso = getLast7Days()[0];

  const [profileRes, dietRes, scores, todayLogRes, gains, bodyAgeRes, rosterRes, weekFoodRes, weekWorkoutRes] =
    await Promise.all([
  ...
  // Build sets of dates with activity for quick lookup
  const foodDates = new Set((weekFoodRes.data ?? []).map((r) => r.date));
  const workoutDates = new Set((weekWorkoutRes.data ?? []).map((r) => r.date));
  ...
  if (activeRoster) {
    ...
    // Batch: one query for all check-in responses instead of N+1
    const checkinIds = (checkinsRes.data ?? []).map((c) => c.id);
    let allLastResponses: { checkin_id: string; submitted_at: string }[] = [];
    if (checkinIds.length > 0) {
      ...
      allLastResponses = data ?? [];
    }

    // Map checkin_id → most recent submission (already sorted desc, so first seen wins)
    const lastSubmissionMap = new Map<string, string>();
    for (const r of allLastResponses) {
      if (!lastSubmissionMap.has(r.checkin_id)) {
        lastSubmissionMap.set(r.checkin_id, r.submitted_at);
      }
    }

    const dueDays: Record<string, number> = { daily: 1, weekly: 7, biweekly: 14, monthly: 30 };
    const pendingCheckins = (checkinsRes.data ?? [])
      .filter((c) => {
        const lastAt = lastSubmissionMap.get(c.id);
        const diffDays = lastAt
          ? Math.floor((Date.now() - new Date(lastAt).getTime()) / 86_400_000)
          : 999;
        return diffDays >= (dueDays[c.frequency] ?? 999);
      })
  ...
  return (
    <div className="flex flex-col min-h-full">
      ...
      <div className="flex-1 px-8 py-6 space-y-6">
          ...
          {gains.hasData ? (
            <>
              <div className="flex flex-col sm:flex-row sm:items-center gap-6">
                  ...
                  <div>
                    ...
                    {gains.trend !== 0 && (
                      <p
                        ...
                        style={{
                          color:
                            gains.trend > 0 ? "var(--color-accent)" : "var(--color-text-muted)",
                        }}
                      >
                  ...
                  {gains.pillars.map((p) => (
                    ...
                    >
                      ...
                      <p
                        ...
                        style={{
                          color:
                            p.score == null ? "var(--color-text-muted)" : "var(--color-text)",
                        }}
                      >
                ...
                {gains.topLever && (
                  <div
                    className="shrink-0 sm:max-w-52 rounded-lg px-4 py-3"
        ...
        {coachingData ? (
          ...
          >
            ...
            <div className="flex items-center gap-4 flex-wrap">
              {/* Coach avatar + name */}
              <Link
                href={`/creator/${coachingData.creatorSlug}`}
                style={{ textDecoration: "none", display: "flex", alignItems: "center", gap: 12 }}
              >
                {coachingData.creatorAvatar ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img
                    src={coachingData.creatorAvatar}
                    alt={coachingData.creatorName}
                    style={{
                      width: 44,
                      height: 44,
                      borderRadius: "50%",
          ...
          <Link href="/discover" style={{ textDecoration: "none" }}>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p
                  className="text-sm font-bold"
                  style={{ color: "var(--color-text)", margin: "0 0 2px" }}
                >
                  Find a coach
                </p>
                <p className="text-xs" style={{ color: "var(--color-text-muted)", margin: 0 }}>
        ...
        {/* Main grid: macro targets + weekly activity */}
        <div className="grid lg:grid-cols-5 gap-6">
          ...
          {/* This week */}
          <div className="lg:col-span-3 rounded-xl border p-5 flex flex-col gap-5"
            style={{ background: "var(--color-surface)", borderColor: "var(--color-border)" }}
          >
                ...
                return (
                  <div key={date} className="flex flex-col items-center gap-1.5">
                    ...
                    <div
                      ...
                      style={{
                        background: hasFood ? "rgba(74,222,128,0.15)" : "var(--color-bg)",
                        border: `1px solid ${hasFood ? "rgba(74,222,128,0.4)" : "var(--color-border)"}`,
                    ...
                    >
                      {hasFood ? (
                        ...
                        <UtensilsCrossed size={11} style={{ color: "var(--color-border)" }} />
                      )}
                    </div>
                    {/* Workout dot */}
                    <div
                      className="size-8 rounded-lg flex items-center justify-center"
                      style={{
                        background: hasWorkout ? "rgba(96,165,250,0.15)" : "var(--color-bg)",
                        border: `1px solid ${hasWorkout ? "rgba(96,165,250,0.4)" : "var(--color-border)"}`,
                      }}
                    >
                      {hasWorkout ? (
                        <CheckCircle2 size={14} style={{ color: "#60a5fa" }} />
                      ) : (
                        <Dumbbell size={11} style={{ color: "var(--color-border)" }} />
                      )}
                    </div>
                  </div>
                );
            ...
            <div className="flex items-center gap-5 pt-1">
              <div className="flex items-center gap-1.5">
                <div className="size-2.5 rounded-sm" style={{ background: "#4ade80" }} />
                <span className="text-[10px]" style={{ color: "var(--color-text-muted)" }}>
                  Food logged
                </span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="size-2.5 rounded-sm" style={{ background: "#60a5fa" }} />
                <span className="text-[10px]" style={{ color: "var(--color-text-muted)" }}>
                  Workout completed
                </span>
              </div>
            ...
            {/* Contextual CTAs */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1 border-t" style={{ borderColor: "var(--color-border)" }}>
              <Link
                href="/nutrition/log"
              ...
              >
                <div
                  ...
                  style={{ background: foodDates.has(todayIso) ? "rgba(74,222,128,0.1)" : "var(--color-accent-subtle)" }}
                >
                  <UtensilsCrossed
                    size={14}
                    style={{ color: foodDates.has(todayIso) ? "#4ade80" : "var(--color-accent)" }}
                  />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold" style={{ color: "var(--color-text)" }}>
                    {foodDates.has(todayIso) ? "Update today's log" : "Log today's meals"}
                  </p>
                  <p className="text-[10px]" style={{ color: "var(--color-text-muted)" }}>
                    {foodDates.has(todayIso) ? "You've logged food today" : "No entries yet"}
                  </p>
                </div>
              ...
              >
                <div
                  className="size-8 rounded-lg flex items-center justify-center shrink-0"
                  style={{
                    background: workoutDates.has(todayIso) ? "rgba(96,165,250,0.1)" : "var(--color-surface-elevated)",
                  }}
                >
                  <TrendingUp
                    size={14}
                    style={{ color: workoutDates.has(todayIso) ? "#60a5fa" : "var(--color-text-secondary)" }}
                  />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold" style={{ color: "var(--color-text)" }}>
                    {workoutDates.has(todayIso) ? "Workout done" : "Log a workout"}
                  </p>
                  <p className="text-[10px]" style={{ color: "var(--color-text-muted)" }}>
                    {workoutDates.has(todayIso) ? "Completed today" : "Start or build a plan"}
                  </p>
                </div>
                <ArrowRight size={13} style={{ color: "var(--color-text-muted)", flexShrink: 0 }} />
              </Link>
...
function BodyAgeRow({
  ...
  bodyAge: { body_age_score: number | null; chronological_age: number | null } | null;
}) {
  return (
    <div
    ...
    >
      <div className="flex items-center gap-2.5">
        <HeartPulse
          size={14}
          style={{ color: bodyAge?.body_age_score ? "#a78bfa" : "var(--color-text-muted)" }}
        />
        <span className="text-xs font-semibold" style={{ color: "var(--color-text-muted)" }}>
          Body Age
        </span>
        {bodyAge?.body_age_score ? (
          <>
            <span
            ...
            {bodyAge.chronological_age && (() => {
              ...
              return (
                <span
                  ...
                  style={{
                    color:
                      delta < -2 ? "#4ade80" : delta > 2 ? "#f87171" : "var(--color-text-secondary)",
                  }}
                >
                  {delta < -2 ? (
                    <><TrendingDown size={12} />{Math.abs(delta)}y younger</>
                  ) : delta > 2 ? (
                    <><TrendingUp size={12} />{delta}y older</>
                  ) : (
