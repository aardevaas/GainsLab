...
import type { ArticleCategory, EducationArticle } from '@/lib/learn/types';
...
async function getArticle(slug: string): Promise<EducationArticle | null> {
  ...
  const { data } = await supabase
    ...
    .single();
  return data as unknown as EducationArticle | null;
}
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="flex items-center gap-1.5" style={{ color: 'var(--color-text-muted)' }}>
              <Clock size={12} />
              <span className="text-xs">{article.reading_time} min read</span>
            </div>
          </div>
