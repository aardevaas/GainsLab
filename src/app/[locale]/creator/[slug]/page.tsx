...
export default async function CreatorProfilePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Stored values are validated at write time (see studio/settings/actions.ts),
  // but re-checking here means a row written before that guard existed (or via
  // a future write path) still can't render a javascript: URL.
  const instagramUrl = creator.instagram_url ? safeHttpUrl(creator.instagram_url) : null;
  const youtubeUrl = creator.youtube_url ? safeHttpUrl(creator.youtube_url) : null;
  const tiktokUrl = creator.tiktok_url ? safeHttpUrl(creator.tiktok_url) : null;
  const websiteUrl = creator.website_url ? safeHttpUrl(creator.website_url) : null;
    ...
    <div style={{ background: 'var(--color-bg)', minHeight: '100dvh' }}>
      ...
      <div style={{ maxWidth: 980, margin: '0 auto', padding: '0 24px 100px' }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 22, flexWrap: 'wrap' }}>
            ...
            <div style={{ position: 'relative', flexShrink: 0 }}>
              ...
              {creator.is_verified && (
                <div style={{
                  ...
                  background: TIER_COLORS[creator.verification_tier as VerificationTier], border: '2px solid var(--color-surface)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                }}>
            ...
            <div style={{ flex: 1, minWidth: 200 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginBottom: 6 }}>
                ...
                {(creator.verification_tier === 'pro' || creator.verification_tier === 'elite') && (
                  <span style={{
                    fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase',
                    padding: '3px 8px', borderRadius: 4, fontFamily: 'var(--font-mono)',
                    background: `${TIER_COLORS[creator.verification_tier as VerificationTier]}18`,
                    color: TIER_COLORS[creator.verification_tier as VerificationTier],
                    border: `1px solid ${TIER_COLORS[creator.verification_tier as VerificationTier]}35`,
                  }}>
                    {TIER_LABELS[creator.verification_tier as VerificationTier]}
                  </span>
                )}
                {creator.is_featured && (
                  <span style={{
          ...
          {/* Social links */}
          {(instagramUrl || youtubeUrl || tiktokUrl || websiteUrl) && (
